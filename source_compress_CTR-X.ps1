...
$timespan = new-timespan -Seconds 60
	...
	Write-Output "########################################################################################################################`r`n                                        Created on $dateHeader                          `r`n########################################################################################################################" | Add-Content "$archives\$dateLog.log"
	Write-Output "$date - Temporary workpath $tempFinalFolder created!`r`n" | Add-Content "$archives\$dateLog.log"
	...
	Write-Output "$date - $numFiles files to be processed now!" | Add-Content "$archives\$dateLog.log"
...
#region temporary workpath for zipping
	...
	{ 
		#Write-Output "$date - File $in found!" | Add-Content "$archives\$dateLog.log"
		Copy-Item "$in" -destination $tempFinalFolder -Force 
		Write-Output "$date - File $in moved to temporary workpath $tempFinalFolder" | Add-Content "$archives\$dateLog.log"
	}
	...
	Write-Output "`r`n$date - Zip file $zipTo created!" | Add-Content "$archives\$dateLog.log"
	...
	{		
		...
		Write-Output "$date - Zip file $zip moved to $tempFolderDestin`r`n" | Add-Content "$archives\$dateLog.log"
	}
...
#region cleaning up files already zipped
	...
	foreach($in in $inFiles)
	{
		...
		Write-Output "$date - Collected file $in moved to $archives" | Add-Content "$archives\$dateLog.log"		
	}
		
	#remove temporary folder on each source dir
	Remove-Item $tempFinalFolder -Recurse
	Write-Output "`r`n$date - Temporary workpath $tempFinalFolder deleted!" | Add-Content "$archives\$dateLog.log"
...
#region check new files for next run
	...
	Write-Output "`r`n$date - $numFilesNewCount new files found to be collected on $tempFolderRoot`r`n" | Add-Content "$archives\$dateLog.log"
